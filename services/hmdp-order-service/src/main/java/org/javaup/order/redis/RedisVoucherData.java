package org.javaup.order.redis;
...
import org.javaup.order.lua.SeckillVoucherRollBackOperate;
...
/**
 * @description: Redis 秒杀订单回滚数据操作组件（order-service 版本）
 * @maintainer: lrb
 **/
@Slf4j
@Component
public class RedisVoucherData {
    ...
    public void rollbackRedisVoucherData(SeckillVoucherOrderOperate seckillVoucherOrderOperate,
        ...
        if (!ok) {
            ...
            log.error("Redis回滚最终失败|voucherId={}|userId={}|orderId={}|traceId={} reason={}",
                    voucherId, userId, orderId, traceId, reason);
            saveRollbackFailureLog(voucherId, userId, orderId, traceId,
                    "redis rollback failed after retries: " + reason, finalCode);
            safeInc("seckill_rollback_retry_give_up", "component", "redis_voucher_data");
        }
    }

    private Integer luaRollbackWithResultCode(List<String> keys, String[] args,
    ...
    private void saveRollbackFailureLog(Long voucherId, Long userId, Long orderId,
                                        Long traceId, String detail, Integer resultCode) {
        try {
